...
**Domain:** `Supply Chain / Procurement` · **Suite:** `Supply Chain Suite` · **Accent:** `#fdba74`
...
**Supplier Email Parser** is a standalone, browser-based AI workflow app for **Supply Chain / Procurement**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `supplier_thread`<br>Supplier thread | text | Yes | — | Affects input: Supplier thread. |
...
| `supplier_profiles`<br>supplier profiles | select | No | — | Affects customization: supplier profiles. |
| `po_template`<br>PO template | text | No | — | Affects customization: PO template. |
| `tolerance_rules`<br>tolerance rules | slider | No | 50 | Affects customization: tolerance rules. |
| `payment_terms`<br>payment terms | text | No | — | Affects customization: payment terms. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9157
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9157`. |
...
- Supplier thread
- structured PO/procurement data
...
```bash
curl http://127.0.0.1:9157/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9157/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9157 uvicorn app.main:app --host 0.0.0.0 --port 9157
```
...
- `plugins/supplier-email-parser.json` for plugin metadata.
